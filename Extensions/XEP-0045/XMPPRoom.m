...
enum StatusCode
{
    kXMPPStatusCodeRoomCreated = 201,
    kXMPPStatusCodeBanned = 301,
    kXMPPStatusCodeNewNickName = 303,
    kXMPPStatusCodeKicked = 307,
    kXMPPStatusCodeRemovedAffiliateChange = 321,
};
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    // 307 - Inform all occupants of kicked user.
    // 321 - Affiliate change.
    ...
    BOOL isKicked = NO;
    BOOL isBanned = NO;
    BOOL removedAffiliate = NO;
    
    // NOTE: Maybe this should be an array
    NSMutableArray* statusCodes = [NSMutableArray array];
    
    for (NSXMLElement *status in [x elementsForName:@"status"])
    {
        [statusCodes addObject:@([status attributeIntValueForName:@"code"])];
        switch ([status attributeIntValueForName:@"code"])
        {
            case 110 : isMyPresence = YES;     break;
            case 201 : didCreateRoom = YES;    break;
            case 210 :
            case 301 : isBanned = YES; break;
            case 303 : isNicknameChange = YES; break;
            case 307 : isKicked = YES; break;
        }
    }
    ...
    if (isMyPresence)
    {
        ...
        {
            BOOL handledLeave = NO;
            
            // User was kicked from the room
            if ([statusCodes containsObject:@(kXMPPStatusCodeKicked)]) {
                
                
                [multicastDelegate xmppRoom:self kickedWithReason:@""];
            ...
            if ([statusCodes containsObject:@(kXMPPStatusCodeBanned)]) {
                // Check if this occupant was banned
                NSString *reason = @"";
                
                [multicastDelegate xmppRoom:self bannedWithReason:reason];
                
                
                handledLeave = YES;
            }
            
            // User's membership was revoked from the room
            if ([statusCodes containsObject:@(kXMPPStatusCodeRemovedAffiliateChange)]) {
                // Check if this occupant's membership was revoked
                [multicastDelegate xmppRoomMembershipRevoked:self];
                handledLeave = YES;
            }
        ...
        else if (isUnavailable)
        {
            ...
            if ([statusCodes containsObject:@(kXMPPStatusCodeKicked)]) {
                ...
                if (isMyPresence) {
                    ...
                    // Other user is kicked from room
                    [multicastDelegate xmppRoom:self occupantKicked:presence.from withReason:@""];
                }
            ...
            if ([statusCodes containsObject:@(kXMPPStatusCodeBanned)]) {
                ...
                if (isMyPresence) {
                    ...
                    [multicastDelegate xmppRoom:self occupantBanned:presence.from withReason:reason];
                }
            ...
            if ([statusCodes containsObject:@(kXMPPStatusCodeNewNickName)]) {
                //                [multicastDelegate xmppRoom:self nicknameChanged:]
            }
